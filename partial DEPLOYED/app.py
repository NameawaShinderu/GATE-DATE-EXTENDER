...
difference = (current_date - july_8).days
...
def generate():
    user_date_str = request.form.get('user_date')
    ...
    # Load the Excel file again to get the updated data
    workbook = openpyxl.load_workbook('your_file_updated.xlsx')
    sheet = workbook.active
    data = []
    for row in sheet.iter_rows(values_only=True):
        data.append(row)

    return render_template('result.html', dates=updated_dates, excel_data=data)
...
def today():
    ...
    # Read the updated Excel data directly
